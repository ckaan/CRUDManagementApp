...
from clinic.dao.turnovers.tnote_dao_pickle import NoteDAO
from clinic.dao.turnovers.turnovers_dao_json import TurnoversDAO
...
class TController:
    ...
    def __init__(self, current_building,filepath, autosave=True):
        ...
        patients_path = os.path.join(base, "turnovers.json")
        records_path = os.path.join(base, "records")

        self.autosave = autosave
        self.logged_in = True
        self.user_dao = UserDAO(json_file_path = users_path)
        self.patient_dao = TurnoversDAO(autosave=autosave, filepath=patients_path)
    ...
    def get_turnovers_for_building(self, building_id):
        """
        ...
        if building_id in self.patient_dao.patients:
            return self.patient_dao.patients[building_id].turnovers  # ✅ Fetch existing turnovers
        else:
            return []  # ✅ Create empty list if no turnovers exist
    ...
    def create_patient(self, group_code, code, company_name, authority, bank_name, taxno, taxplace, iban, address, phonenumber, email, web):
        ...
        patient =self.patient_dao.create_patient(group_code, code, company_name, authority, bank_name, taxno, taxplace, iban, address, phonenumber, email, web)
        print(f"Patient with SIN {group_code} added: {patient}")
        return patient
    ...
    def search_patient(self, group_code):
        self._ensure_logged_in()
        return self.patient_dao.search_patient(group_code) 
        '''
    ...
    def update_patient(self, patient_id, group_code, code, company_name, authority, bank_name, taxno, taxplace, iban, address, phonenumber, email, web):
        ...
        return self.patient_dao.update_patient(patient_id, group_code, code, company_name, authority, bank_name, taxno, taxplace, iban, address, phonenumber, email, web)
    ...
    def delete_patient(self, group_code):
        ...
        return self.patient_dao.delete_patient(group_code)
    ...
    def set_current_patient(self, group_code):
        self._ensure_logged_in()  # Ensure the user is logged in

        patient = self.patient_dao.search_patient(group_code)
